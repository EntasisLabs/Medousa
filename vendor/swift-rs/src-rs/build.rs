...
impl SwiftLinker {
    ...
    pub fn link(self) {
        ...
        for package in self.packages {
            ...
            // `--triple` makes SwiftPM own the complete Xcode 27 cross-build.
            // Passing the target through globally as well leaks the iOS triple
            // into host-only macro/tool targets (for example
            // ArgumentParserToolInfo), where clang then rejects host flags such
            // as `-mcx16` for arm64 iOS. Older Xcode still needs the explicit
            // C/C++ overrides alongside the per-swiftc target overrides.
            if !use_triple {
                command
