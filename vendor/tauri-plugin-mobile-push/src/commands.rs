    ...
    #[cfg(target_os = "ios")]
    {
        ...
        std::thread::spawn(move || {
            ...
            let granted = match result {
                Ok(code) => code == 1,
                Err(_) => {
                    eprintln!("[mobile-push] request_permission FFI panicked");
                    false
                }
            };
            eprintln!("[mobile-push] FFI returned: {}", granted as i32);
        ...
        std::thread::spawn(move || {
            ...
            let send_result = std::panic::catch_unwind(|| {
                ...
                if result > 0 {
                    ...
                    String::from_utf8(bytes).map_err(|e| format!("Invalid UTF-8 token: {}", e))
                } else if result == -2 {
                    Err("APNs token request timed out".to_string())
                } else {
                    Err("Failed to get APNs device token".to_string())
                }
            });
            match send_result {
                Ok(token_result) => {
                    let _ = tx.send(token_result);
                }
                Err(_) => {
                    eprintln!("[mobile-push] get_token FFI panicked");
                    let _ = tx.send(Err("APNs token FFI panicked".to_string()));
                }
            }
        });
