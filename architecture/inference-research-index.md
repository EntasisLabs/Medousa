...
| [Vulkan memory budget](https://docs.vulkan.org/refpages/latest/refpages/source/VkPhysicalDeviceMemoryBudgetPropertiesEXT.html) | Upstream | `heapBudget` is the process allocation guideline and includes current allocations; `heapUsage` estimates current process usage | Normalize remaining headroom as budget minus usage and never substitute heap size when the current budget is tighter. |
| [DXGI video-memory budget](https://learn.microsoft.com/en-us/windows/win32/api/dxgi1_4/nf-dxgi1_4-idxgiadapter3-queryvideomemoryinfo) | Upstream | WDDM current process budget and usage for local/non-local segments | Native Windows admission must use `Budget - CurrentUsage`, track budget changes, and avoid treating physical VRAM as the process allocation allowance. |
