...
| [NVML API](https://docs.nvidia.com/deploy/nvml-api/) | Upstream | Driver-shipped, toolkit-independent device/process memory, utilization, temperature, power, clocks, and throttle telemetry | Native identity and physical/current-process memory now drive NVIDIA admission; qualify the remaining counters and WDDM behavior across the consumer matrix. |
| [nvidia-smi selective queries](https://docs.nvidia.com/deploy/nvidia-smi/index.html#selective-query-options) | Upstream | CSV queries over the same NVML-backed device and compute-process counters | Keep as a lower-priority compatibility and diagnostic lane while the native collector grows; never make CLI parsing the resource-lease authority when NVML is healthy. |
...
| [AMD SMI](https://rocm.docs.amd.com/projects/amdsmi/en/latest/) | Upstream | Device/process VRAM and GTT, utilization, power, temperature, clock, and RAS telemetry; ABI majors identify breaking header changes | Native major-26 identity and resource evidence now drive Linux AMD admission; qualify older supported majors explicitly instead of assuming struct compatibility. |
| [AMD SMI CLI JSON](https://rocm.docs.amd.com/projects/amdsmi/en/latest/how-to/amdsmi-cli-tool.html) | Upstream | JSON-capable device, monitor, and process telemetry | Keep tolerant JSON as the compatibility lane when the native library is missing, unhealthy, or an unqualified ABI major; preserve every missing field. |
...
| [Vulkan memory budget](https://docs.vulkan.org/refpages/latest/refpages/source/VkPhysicalDeviceMemoryBudgetPropertiesEXT.html) | Upstream | `heapBudget` is the process allocation guideline and includes current allocations; `heapUsage` estimates current process usage | Implemented as a loader-optional, telemetry-only Vulkan 1.1 collector over device-local heaps. Admission uses budget minus usage, never heap size when the current budget is tighter, and never applies Vulkan usage to CUDA/HIP. |
| [DXGI video-memory budget](https://learn.microsoft.com/en-us/windows/win32/api/dxgi1_4/nf-dxgi1_4-idxgiadapter3-queryvideomemoryinfo) | Upstream | WDDM current process budget and usage for local/non-local segments | Implemented for hardware adapters' local segment: native Windows admission uses `Budget - CurrentUsage`, ranks it above vendor physical-memory counters, and keeps dedicated VRAM distinct. |
