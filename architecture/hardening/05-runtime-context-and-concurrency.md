...
H05.0–H05.6 are implemented. Every daemon, TUI, recurring, ingest, heartbeat,
and durable-worker turn is admitted with a daemon-issued `TurnHandle` and an
immutable `TurnExecutionContext`. The bounded registry and per-turn task group
own exact generations, cancellation roots, deadlines, and cleanup leases.
Provider/tool leaves fail closed before polling when context is absent; the
violation count, live count, and high-water mark are exposed by `/v1/stats`.

The process-global tool sink, shared `TuiRuntime::turn_scope`, scope
save/install/restore sequences, runtime-session bootstrap authority fallback,
worker current-runtime/current-session slots, and global grapheme source are
deleted. The remaining Tokio task-local projections are one-way compatibility
adapters for upstream tool traits: they derive only from the admitted immutable
context, have no process-global fallback, and are reinstalled explicitly for
owned child futures.

Workers use bounded parent and live-execution registries keyed by opaque parent
handles and exact `work_id` generations. Bound-workshop admission is atomic per
session; steer, cancel, completion, and stale lease cleanup compare the named
generation. Durable continuation records are versioned, single-claim, and
reauthorized against current profile/session visibility before a fresh bounded
execution is admitted.

Provider completions, tool catalog reads, sequential and parallel tool calls,
fallback synthesis, worker calls, and stream pumps are cancellation/deadline
bounded. Parallel tool tasks reinstall the exact parent context. Stream and
attempt pumps abort on owner drop while normal completion drains in order.

Home's four singleton reply mailboxes are replaced by one bounded request
broker. Every desktop snapshot, action, navigation-state, and find callback
carries request ID and surface identity and is checked against response kind
and navigation generation. RAII cleanup covers evaluation errors, timeouts,
caller drop, navigation, tab replacement, and close. Payload-free diagnostics
report pending/high-water and every match/rejection class through Tauri IPC.

Deterministic two-turn, two-worker, same-session generation, and browser
reversed-completion fixtures freeze the ownership model. CI rejects the deleted
singleton/optional-context patterns. Supported packaged-platform validation
remains the release gate; it is evidence collection, not an open ownership
design or compatibility path.

The current H05.0 request-state inventory is:

| State | Classification | Current action |
| --- | --- | --- |
| Ambient tool sink | Per-turn request state | Process-global slot deleted; immutable task-scoped projection only |
| Shared `TurnContinuationScope` | Legacy projection required by upstream tool traits | Mutable shared holders deleted; projection derives from the admitted context and fails closed when absent |
| Worker scheduler `runtime_ctx` and `bus_session` | Per-parent/worker request state formerly held in two process-wide `Option` slots | Deleted; bounded keyed parent/live-worker registries and exact leases shipped |
| Browser `SNAPSHOT_TX`, `ACT_TX`, `NAV_STATE_TX`, `FIND_TX` | Per-request reply state formerly held in four singleton mailboxes | Deleted; bounded correlated broker and diagnostics shipped |
| `LAST_GRAPHEME_SOURCE` | Per-invocation source formerly selected through one global last-value slot | Moved into the exact `TurnExecutionContext` generation |
...
| Browser placement/viewport and per-surface active-tab slots | UI/surface state, not request/reply authority | Retained as explicitly keyed surface state; reply ownership lives in `BrowserHostState` |
...
## Resolved ownership failures

### Turn and tool context

Before H05, `TuiRuntime` owned one `Arc<RwLock<Option<TurnContinuationScope>>>` and passed it
...
The former `engine_adapters::ACTIVE_TOOL_SINK` was described as per-turn but was one
...
The former `TurnWorkerScheduler` had one `runtime_ctx` and one `bus_session`. The
...
The former `human_browser.rs` declared global `SNAPSHOT_TX`, `ACT_TX`, `NAV_STATE_TX`, and
...
| Snapshot response | 8 MiB callback maximum before conversion; smaller caller `max_chars` still applies |
...
**Implemented with one upstream-trait adapter:** the adapter is task-local,
immutable, fail-closed, and covered by the CI singleton guard.
...
**Implemented for code and CI; packaged-platform evidence remains the release
validation step.**
