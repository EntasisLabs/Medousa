# Polish & Package — capability era (historical)

> **Status:** Archived (2026-07) — superseded by [../polish-and-package-plan.md](../polish-and-package-plan.md) (0.6 felt polish)  
> **Era:** Pre–0.6 “expose the engine” checklist (P0–P7). Most items shipped; do not treat as current backlog.  
> **Related:** [turn-runtime-and-lanes.md](../turn-runtime-and-lanes.md), [identity-manuscripts-and-recall-plan.md](../identity-manuscripts-and-recall-plan.md), [iroh-p2p-pairing-plan.md](../iroh-p2p-pairing-plan.md), [media-and-attachments-plan.md](../media-and-attachments-plan.md), [desktop-distribution-plan.md](../desktop-distribution-plan.md), [continuity-first-redesign.md](continuity-first-redesign.md)
...
**Status:** 🔄 P0.1 / P0.2 / P0.3 / P0.5 shipped; **P0.4 Iroh smoke** still open

| ID | Deliverable | Type | Acceptance | Status |
|----|-------------|------|------------|--------|
| P0.1 | Engine auto-start after wizard | Polish | Fresh `.app` install → chat works with **zero terminal**; doctor green | ✅ `ensureWorkshopEngineHealthy` in `connectWorkshop` / `reconnectWorkshop` |
| P0.2 | Home **Workshop health** chip | Exposure | Settings shows engine version, last turn, restart affordance | ✅ Settings Basement |
| P0.3 | Fix known sidecar wiring bugs | Polish | Track in issues; smoke: sleep/wake, force-quit, relaunch | ✅ resume path re-spawns when unhealthy |
| P0.4 | Iroh Phase 0 smoke + Phase 2 mobile | Polish | [iroh-p2p-pairing-plan.md](iroh-p2p-pairing-plan.md) — QR → `/health` over tunnel | ⬜ |
| P0.5 | Connection runbook in product | Exposure | Link Settings Diagnostics → [connection-reliability](../docs/runbooks/connection-reliability.md) | ✅ bundled under `src-tauri/resources` + `resource_dir` resolve |

**Code anchors:** `ensureWorkshopEngine.ts`, `workshopConnection.ts`, `apps/medousa-home/src-tauri/` sidecar scripts, `prepare-engine-sidecar.sh`, `medousa_paths.rs` `connection_runbook_path`, Settings Diagnostics panels.
