...
fn resolve_slack_bot_token(explicit: Option<&str>) -> Result<String> {
    explicit
        ...
        .ok_or_else(|| {
            anyhow!(
                "missing slack bot token: pass --bot-token or set MEDOUSA_SLACK_BOT_TOKEN / SLACK_BOT_TOKEN"
            )
        })
}
...
fn resolve_slack_app_token(explicit: Option<&str>) -> Result<String> {
    explicit
        ...
        .ok_or_else(|| {
            anyhow!(
                "missing slack app token: pass --app-token or set MEDOUSA_SLACK_APP_TOKEN / SLACK_APP_TOKEN (xapp-… for Socket Mode)"
            )
        })
}
