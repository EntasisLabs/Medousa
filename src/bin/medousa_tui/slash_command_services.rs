...
pub(crate) async fn handle_model_command(
    ...
    match execute_runtime_config_command_via_daemon(&state.daemon_url, request).await {
        ...
        Err(err) => {
            push_obs_alert(state, format!("⚠ model command failed: {err}"));
        }
    }
...
pub(crate) async fn handle_depth_command(
    ...
    match execute_runtime_config_command_via_daemon(&state.daemon_url, request).await {
        ...
        Err(err) => {
            push_obs_alert(state, format!("⚠ depth command failed: {err}"));
        }
    }
...
pub(crate) fn handle_perf_command(
    ...
    match sub {
        ...
        "reset" => {
            ...
            push_obs_alert(state, "✓ perf counters and baseline reset".to_string());
        }
        _ => {
