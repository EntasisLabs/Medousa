...
fn build_hints(
    ...
    if diagnostics.is_some_and(|opts| opts.include_runtime || opts.include_static_lint) {
        hints.push(
            "Widget runtime issues appear under components[].runtime — presentation fixes via cognition_store_write action=artifacts.write; media_embed has no artifact lint."
                .to_string(),
        );
    }
    hints
