...
use super::catalog::CatalogModelEntry;
use super::hardware::medousa_data_dir;
...
impl ModelStore {
    ...
    pub async fn start_download(&self, entry: CatalogModelEntry) -> Result<ModelDownloadProgress, String> {
...
pub fn local_repo_if_installed(model_id: &str) -> Option<String> {
    read_models_index()
        .ok()
        .and_then(|index| index.models.get(model_id).map(|record| record.local_path.clone()))
}
...
async fn download_hf_file(
    ...
    let existing = fs::metadata(destination).await.map(|meta| meta.len()).unwrap_or(0);
    let url = format!(
...
async fn run_download_job(
    ...
    store.index.write().await.models.insert(entry.id.clone(), record);
...
mod tests {
    ...
    fn include_hf_file_filters_docs() {
        ...
        assert!(include_hf_file("weights/model.q4k.uqff"));
