...
//! - `cognition_turn action=turn.finish` ends immediately and appends onto one
//!   held response;
...
pub fn continue_control_message(reason: ContinueReason, _missing_tools: &[String]) -> String {
    match reason {
        ...
        ContinueReason::PackHold => {
            crate::turn_policy::pack_hold_resolution_control_message()
        }
    }
}
...
fn decide_host_after_tools_text_round(ctx: &AfterToolsRoundContext) -> TurnRoundAction {
...
fn decide_foreground_after_tools_text_round(ctx: &AfterToolsRoundContext) -> TurnRoundAction {
    let draft = ctx.draft_text.trim();
...
mod event_driven_tests {
    ...
    fn after_tools(draft: &str, profile: TurnCompletionProfile) -> AfterToolsRoundContext {
    ...
    #[test]
    fn host_empty_after_tools_gets_one_bounded_recovery_round() {
        let mut context = after_tools("", TurnCompletionProfile::HostScheduler);
        assert!(matches!(
        ...
        context.empty_after_tools_continues_used = HOST_EMPTY_AFTER_TOOLS_CONTINUE_CAP;
        assert!(matches!(
            decide_after_tools_text_round(&context),
            TurnRoundAction::EndTurn {
                termination_reason: "max_rounds_fuse"
