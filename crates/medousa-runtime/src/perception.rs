...
//! current tool loop. Optional evidence persistence enters through a focused port.
...
use crate::ports::{PerceptionEvidencePort, PerceptionEvidenceRequest};
...
pub struct ToolPerceptionGovernor {
    ...
    evidence_port: Option<Arc<dyn PerceptionEvidencePort>>,
}
...
impl ToolPerceptionGovernor {
    pub fn new(evidence_port: Option<Arc<dyn PerceptionEvidencePort>>) -> Self {
        Self {
            evidence_port,
            ..Default::default()
        }
    ...
    fn persist_evidence(
        ...
        let evidence_port = self.evidence_port.as_ref()?;
        match evidence_port.persist(PerceptionEvidenceRequest {
            tool_name,
            source_call_id,
            output,
            failed: is_failure(output),
        }) {
            Ok(evidence) => {
                if let Some(err) = evidence.receipt_stage_error.as_deref() {
                    add_metric(
                ...
                Some(json!({
                    ...
                    "durable_receipt_staged": evidence.durable_receipt_staged,
...
fn tool_payload_is_requeryable(tool_name: &str) -> bool {
    tool_name.starts_with("cognition_code_")
        || matches!(
            tool_name,
