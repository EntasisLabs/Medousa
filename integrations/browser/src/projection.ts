...
export type ProjectedEvent =
  ...
  | { kind: "tool_started"; name: string; runId: string }
  | { kind: "tool_finished"; name: string; runId: string; status: string }
...
export function projectStreamEvent(
  ...
  if (event.permission_request_id) {
    projected.push({
      ...
      message: event.operator_message ?? event.message ?? "Medousa needs permission to continue.",
    });
  }
  ...
  if (eventType === "tool_started" || (event.tool_name && event.tool_status === "running")) {
    ...
    projected.push({ kind: "tool_started", name: formatToolName(event.tool_name ?? "tool"), runId });
  }
  ...
  if (isBackgroundHandoffEvent(event)) {
    projected.push({
      ...
      text:
        ...
        (eventType === "worker_ack" ? "Background work started" : "Medousa is in the workshop"),
...
function formatToolName(name: string): string {
  return name.replace(/^cognition_/, "").replaceAll("_", " ").replace(/\b\w/g, (character) => character.toUpperCase());
}

function friendlyError(message: string): string {
  const trimmed = message.trim();
  return !trimmed || isTelemetry(trimmed) ? "Something went wrong on this turn. Try again in a moment." : trimmed;
}
