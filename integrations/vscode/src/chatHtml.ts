...
<html lang="en"><head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .runtime-strip { display: flex; gap: 5px; padding-bottom: 6px; }
    .runtime-button { min-width: 0; display: inline-flex; align-items: center; gap: 4px; border: 1px solid var(--vscode-widget-border); border-radius: 999px; padding: 3px 8px; color: var(--vscode-foreground); background: var(--vscode-editor-background); font-size: 10px; }
    .runtime-button:hover { border-color: var(--vscode-focusBorder); }
    .runtime-button.coder { color: var(--vscode-symbolIcon-classForeground, var(--vscode-textLink-foreground)); }
    .runtime-button.warning { border-color: var(--vscode-inputValidation-warningBorder); }
    .runtime-button span { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
<div class="shell">
  ...
  <footer>
    <div class="runtime-strip"><button id="mode-button" class="runtime-button" title="Choose how Medousa works">✦ <span>General</span></button><button id="work-button" class="runtime-button" title="Bind this conversation to a Forge undertaking">◎ <span>Choose project</span></button></div>
    <div id="context"></div>
...
<script nonce="${nonce}">
  ...
  let modeProposalNode = null;
  let modeProposalTimer = null;
  ...
  function clearModeProposal() { if (modeProposalTimer) clearInterval(modeProposalTimer); modeProposalTimer = null; if (modeProposalNode) modeProposalNode.remove(); modeProposalNode = null; }
  function addModeProposal(message) { clearModeProposal(); const node = document.createElement("div"); node.className = "attention"; const copy = document.createElement("span"); const updateCopy = function() { const seconds = Math.max(0, Math.ceil((Date.parse(message.expiresAt) - Date.now()) / 1000)); const modeLabel = message.toMode === "coder" ? "Coder" : message.toMode === "instant" ? "Instant" : "General"; copy.textContent = "Switch to " + modeLabel + "? " + message.reason + " · expires in " + seconds + "s"; if (seconds === 0) clearModeProposal(); }; const actions = document.createElement("div"); actions.className = "attention-actions"; ["Switch", "Not now"].forEach(function(label) { const button = document.createElement("button"); button.textContent = label; button.addEventListener("click", function() { vscode.postMessage({ type: "modeProposal", requestId: message.proposalId, approve: label === "Switch", toMode: message.toMode }); clearModeProposal(); }); actions.appendChild(button); }); node.appendChild(copy); node.appendChild(actions); messages.appendChild(node); modeProposalNode = node; updateCopy(); if (modeProposalNode) modeProposalTimer = setInterval(updateCopy, 1_000); }
  ...
  modeButton.addEventListener("click", function() { vscode.postMessage({ type: "selectMode" }); });
  workButton.addEventListener("click", function() { vscode.postMessage({ type: "selectUndertaking" }); });
  ...
  window.addEventListener("message", function(event) { const message = event.data;
    ...
    else if (message.type === "modeProposal") addModeProposal(message);
    else if (message.type === "proposalClear") clearModeProposal();
    ...
    else if (message.type === "runtimeState") { modeButton.querySelector("span").textContent = message.modeLabel; modeButton.classList.toggle("coder", message.mode === "coder"); modeButton.classList.toggle("warning", message.mode === "coder" && !message.coderReady); workButton.querySelector("span").textContent = message.workTitle || "Choose project"; workButton.title = message.workId ? "Following " + message.workTitle + " · click to change" : "Bind this conversation to a Forge undertaking"; }
