...
use crate::messaging::daemon_slots;
...
fn start_adapter(
    ...
    match channel {
        "telegram" => {
            let token = daemon_slots::load_kind_slot("telegram", medousa_types::IntegrationSecretSlot::BotToken)
                .ok_or_else(|| "Telegram bot token is missing.".to_string())?;
            apply_telegram_env(&mut command, summary);
            command.arg("--token").arg(token);
        }
        "discord" => {
            let token = daemon_slots::load_kind_slot("discord", medousa_types::IntegrationSecretSlot::BotToken)
                .ok_or_else(|| "Discord bot token is missing.".to_string())?;
            apply_discord_env(&mut command, summary);
            command.arg("--token").arg(token);
        }
        "slack" => {
            let bot_token = daemon_slots::load_kind_slot("slack", medousa_types::IntegrationSecretSlot::BotToken)
                .ok_or_else(|| "Slack bot token is missing.".to_string())?;
            let app_token = daemon_slots::load_kind_slot("slack", medousa_types::IntegrationSecretSlot::AppToken)
