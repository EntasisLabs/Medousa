...
<nav class="code-breadcrumbs flex min-w-0 flex-1 items-center gap-0.5 overflow-x-auto" aria-label="File location">
  {#each segments as segment, index (segment.path)}
    {#if index > 0}
      <ChevronRight size={10} class="shrink-0 text-surface-600" aria-hidden="true" />
    {/if}
    <button
      type="button"
  ...
  {#each symbols as symbol, index (`${symbol.name}:${symbol.line}:${index}`)}
    ...
    <button
      type="button"
      class="code-breadcrumb-seg max-w-40 shrink-0 truncate rounded px-1 py-0.5 text-left text-[11px] text-primary-300/80 hover:bg-surface-800 hover:text-primary-200"
