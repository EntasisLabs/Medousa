...
>
  <div
    class="flex items-center justify-between border-b border-surface-500/30 px-2 py-1.5 text-chrome-md font-medium uppercase tracking-wide text-content-secondary"
  >
    <span>Outline</span>
    {#if onClose}
      <button
  ...
  <ul class="min-h-0 flex-1 overflow-y-auto px-1 py-1 text-chrome-sm">
    {#if symbols.length === 0}
    ...
    {:else}
      {#each symbols as sym (sym.name + sym.line)}
        <li>
          ...
          >
            <span class="shrink-0 text-chrome-sm uppercase text-content-quiet">{sym.kind}</span>
            <span class="min-w-0 truncate text-surface-100">{sym.name}</span>
            <span class="ml-auto shrink-0 text-chrome-sm text-content-quiet">{sym.line}</span>
          </button>
        </li>
      {/each}
    {/if}
