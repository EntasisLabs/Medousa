<script lang="ts">
  ...
  function connectNearby(workshop: DiscoveredWorkshop) {
    error = null;
    success = null;
  ...
  async function submitFallback() {
    ...
    if (!qrUrl) {
      error = "Paste an invite link generated by the workshop owner.";
      return;
    }
    busy = true;
    error = null;
    ...
    {#if nearbyUntrusted.length > 0}
      <div class="peers-nearby-banner" role="status">
        ...
        <div class="peers-nearby-banner-copy">
          ...
          <p class="peers-nearby-banner-meta">Same Wi‑Fi — invite required</p>
        </div>
        <button
          type="button"
  ...
  >
    <div class="peers-sheet peers-sheet-sm" role="dialog" aria-modal="true" aria-label="Connect by address">
      ...
      <p class="peers-sheet-hint">
        Ask the workshop owner to share an invite. Nearby discovery can fill the address, but it cannot read pairing secrets.
      </p>
      ...
      <div class="peers-sheet-actions">
        <button
          type="button"
          class="btn btn-sm btn-primary"
          disabled={busy || !fallbackQrUrl.trim()}
