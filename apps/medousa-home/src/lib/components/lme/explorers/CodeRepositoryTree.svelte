<script lang="ts">
  ...
  async function renameSelected() {
    ...
    const destination = window.prompt("Rename file", path)?.trim();
  ...
  {:else}
    ...
    {#if undertakings.active?.workId === workId && undertakings.active.selectedPath}
      <div class="flex items-center justify-between gap-2 border-b border-surface-500/20 px-2 py-1">
        ...
        <button type="button" class="rounded p-1 text-surface-500 hover:bg-surface-800 hover:text-surface-200" title="Rename selected file" aria-label="Rename selected file" disabled={mutating} onclick={() => void renameSelected()}><Pencil size={11} /></button>
        ...
        {#each rows as row (row.path)}
          ...
          >
            ...
            {#if row.kind === "file" && row.status}
              <span class="shrink-0 font-mono text-[9px] {row.status === '??' ? 'text-emerald-300' : 'text-amber-300'}">{row.status}</span>
            {/if}
          </button>
        {/each}
      </div>
