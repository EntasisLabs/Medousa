...
<section class="flex h-full min-w-0 flex-1 flex-col {visible ? '' : 'hidden'}">
  <header class="border-b border-surface-500/20 px-5 py-4">
    <div class="flex items-start justify-between gap-4">
      <div>
        <h1 class="text-base font-semibold">Runtime</h1>
  ...
  <div class="flex-1 overflow-y-auto px-5 py-4">
    ...
    {#if runtime.activeTab === "now"}
      <div class="space-y-4">
        <section class="rounded-container-token border border-surface-500/20 bg-surface-900/50 p-4">
          <h2 class="text-xs font-medium text-surface-500">Active turn</h2>
          {#if streamingMessage}
            <p class="mt-2 text-sm text-surface-100">
              {formatTurnPhase(streamingMessage.phase ?? "streaming")}
            </p>
            {#if streamingMessage.statusLine}
              <p class="mt-1 text-xs text-surface-400">{streamingMessage.statusLine}</p>
            {/if}
          ...
          {:else}
            <p class="mt-2 text-sm text-surface-400">No active turn</p>
          {/if}
        </section>

        <section class="rounded-container-token border border-surface-500/20 bg-surface-900/50 p-4">
          <h2 class="text-xs font-medium text-surface-500">Workshop pulse</h2>
          <dl class="mt-3 grid grid-cols-2 gap-3 text-xs">
            <div>
              <dt class="text-surface-500">In motion</dt>
              <dd class="mt-0.5 font-mono text-surface-200">{inMotionCount}</dd>
            </div>
            <div>
              <dt class="text-surface-500">Running jobs</dt>
              <dd class="mt-0.5 font-mono text-surface-200">
                {runtime.stats?.running_jobs ?? "—"}
              </dd>
            </div>
            <div>
              <dt class="text-surface-500">Queued</dt>
              <dd class="mt-0.5 font-mono text-surface-200">
                {runtime.stats?.enqueued_jobs ?? "—"}
              </dd>
            </div>
            <div>
              <dt class="text-surface-500">Last scheduler tick</dt>
              <dd class="mt-0.5 font-mono text-surface-200">
        ...
        <section class="rounded-container-token border border-surface-500/20 bg-surface-900/50 p-4">
          <h2 class="text-xs font-medium text-surface-500">Current config</h2>
      ...
      {#if runtime.stats}
        <dl class="grid gap-3 sm:grid-cols-2">
          {#each [
            ...
            <div
              class="rounded-container-token border border-surface-500/20 bg-surface-900/50 p-4"
            >
              <dt class="text-xs text-surface-500">{label}</dt>
    ...
    {:else if runtime.activeTab === "delivery"}
      <div class="space-y-4">
        ...
        {#if runtime.continuations}
          <section class="rounded-container-token border border-surface-500/20 bg-surface-900/50 p-4">
            <h2 class="text-xs font-medium text-surface-500">Continuations</h2>
            <dl class="mt-3 grid grid-cols-2 gap-3 text-xs">
              <div>
                <dt class="text-surface-500">Pending</dt>
                <dd class="mt-0.5 font-mono text-surface-200">
              ...
              <div>
                <dt class="text-surface-500">Resumed</dt>
                <dd class="mt-0.5 font-mono text-surface-200">
                  {runtime.continuations.resumed_count}
                </dd>
              </div>
              <div>
                <dt class="text-surface-500">Consumed</dt>
                <dd class="mt-0.5 font-mono text-surface-200">
                  {runtime.continuations.consumed_count}
                </dd>
              </div>
              <div>
                <dt class="text-surface-500">DLQ pending</dt>
                <dd class="mt-0.5 font-mono text-surface-200">
    ...
    {:else if runtime.activeTab === "controls"}
      <section class="max-w-xl space-y-4">
        ...
        <div class="rounded-container-token border border-surface-500/20 bg-surface-900/50 p-4">
          <h2 class="text-sm font-semibold text-surface-100">Model</h2>
          <div class="mt-4 grid gap-3 sm:grid-cols-2">
            <label class="block text-xs text-surface-400" for="runtime-provider">
              Provider
            </label>
            <label class="block text-xs text-surface-400" for="runtime-model">
              Model
            </label>
            <input
              id="runtime-provider"
        ...
        <div class="rounded-container-token border border-surface-500/20 bg-surface-900/50 p-4">
          ...
          <p class="mt-3 text-xs text-surface-500">{runtime.depthHint()}</p>
        </div>
      ...
      <section>
        ...
        <div class="mt-4 overflow-x-auto">
          <table class="w-full min-w-[32rem] text-left text-xs">
            <thead>
              <tr class="border-b border-surface-500/20 text-surface-500">
            ...
            <tbody>
              {#each runtime.stageRoutes() as route (route.role)}
                <tr class="border-b border-surface-500/10">
                  ...
                  <td class="py-2.5 pr-3 text-surface-400">{route.policy_profile}</td>
                  <td class="py-2.5 text-surface-500">{route.fallback_chain.join(", ")}</td>
                </tr>
              {/each}
            </tbody>
          </table>
