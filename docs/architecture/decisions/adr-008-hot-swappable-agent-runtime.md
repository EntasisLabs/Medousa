...
1. **North star:** Any Medousa **channel** messages through the **daemon** via the **Medousa SDK** (`AgentsApi`). The daemon exposes `/v1/agents/…` for external ACP runtimes (Cursor, Codex, Hermes). Native Medousa stays on `/v1/turns`. External runtimes reach vault/context via **Medousa MCP server**.
...
5. **0.4.0 bones:** list runtimes, create/prompt/stream/cancel, Cursor/Codex spawn-when-available (else stub), thin Home runtime select. Not polished multi-channel pickers. **Later:** Hermes added as a first-class ACP runtime (`hermes-acp` / `hermes acp`).

```text
Channel → Medousa SDK AgentsApi → Daemon /v1/agents
                                      ├── ExternalAcpClient → Cursor / Codex / Hermes
...
- Contract parity includes `agents` accessor rows in the generated [`sdk-contract/openapi.json`](../../../sdk-contract/openapi.json).
