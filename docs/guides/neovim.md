...
- `<leader>mm` switches the conversation between General, Assistant, Teacher, Instant, and Coder.
...
## General, Assistant, Teacher, Instant, and Coder modes
...
evidence-first mentoring policy; Assistant keeps General's conversation and
context while owning accepted work across authorized agents and workshops;
Instant keeps the General execution path while loading a smaller recent context
and tool surface. Coder can be selected before a project exists. Its setup phase
has no file or shell authority.
